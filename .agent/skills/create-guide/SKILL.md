---
name: create-guide
description: Automatically create a new markdown guide, place it in the appropriate folder under public/docs, and update the structure.json index. Use this skill when asked to create a guide, article, tutorial, or register documentation.
---
...
1. Creating category directories under `public/docs/` if they do not exist.
2. Generating a clean, structured Markdown file.
3. Updating `public/docs/structure.json` to register the new file under the correct folder hierarchy so that it dynamically loads in the sidebar.
...
- `--filename` (Required): The name of the markdown file (e.g. `"webgpu-intro.md"`). The script will save it at `public/docs/<category-folder>/<filename>`.
...
2. Create `public/docs/<category-kebab-case>/<filename>`.
3. Open `public/docs/structure.json` and append the new document node under the corresponding category folder. If the category folder is new, add a new folder block:
   ```json
   {
     "name": "Category Name",
     "type": "folder",
